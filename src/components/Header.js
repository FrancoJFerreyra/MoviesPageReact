...
const Header = () => {
	...
	const genreDetails = genreList
		? genreList.map(({ name, id }) => {
				return (
					<li key={id}>
						<Link className='dropdown-item' to={`/genre/${name.toLowerCase()}/page_1`}>
							{name}
						</Link>
					</li>
				);
	...
	const categoriesList = categories.map((category) => {
		return (
			<li key={category}>
				<Link to={`/category/${category.toLowerCase()}/page_1`} className='dropdown-item'>
					{category}
				</Link>
			</li>
		);
		...
		<header className='container-xxl'>
			<Navbar bg='light' expand='lg'>
				...
				<Navbar.Collapse id='navbarScroll'>
					<Nav className='me-auto my-2 my-lg-0' style={{ maxHeight: '100px' }} navbarScroll as='ul'>
						<Nav.Item as='li'>
							<Link
								className='nav-link'
								to='/'
